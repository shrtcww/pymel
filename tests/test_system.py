...
#        cmds.file(new=1, f=1)
    ...
    def test_reference_proxies(self):
        #There should be no proxies
        self.assertTrue(self.sphereRef1.listProxies() == dict())

        #Add a proxy for the sphere as the cone
        self.sphereRef1.addProxy(self.coneFile,'cone')

        #Test list now
        proxies = self.sphereRef1.listProxies()
        self.assertTrue('cone' in proxies)

        #Switch back and forth
        self.sphereRef1.switchToProxy(proxies['cone'])
        self.sphereRef1.switchToProxy(proxies['original'])

        #Remove proxy
        self.removeProxy(proxies['cone'])
        #There should be no proxies
        self.assertTrue(self.sphereRef1.listProxies() == dict())
